...
lane_positions = [200, 440]  # Vasaku ja parema raja keskpunktid (x-koordinaadid).

# Siniste autode andmed (mitu autot korraga).
blue_cars = [{"x": lane_positions[i], "y": random.randint(-500, -50), "speed": base_speed} for i in range(2)]
# Loob kaks sinist autot juhuslike y-positsioonidega ja fikseeritud x-positsioonidega radade keskmetel (vasak ja parem).
...
while running:  # Käivitab peamise mängutsükli.
    ...
    for blue_car_data in blue_cars:
        ...
        if blue_car_data["y"] > screen_height:
            ...
            blue_car_data["x"] = random.choice(lane_positions)  # Auto tuleb kas vasakult või paremalt rajalt.
            blue_car_data["speed"] += speed_increment
            score += 1

        # Joonistab sinise auto täpselt raja keskpunkti (kas vasakult või paremalt rajalt).
